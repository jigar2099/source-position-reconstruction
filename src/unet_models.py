...
class MODELS:
    ...
    def VGG16(self, filt_lst,dns,bins,channel,loss_func,opt,metric):
        ...
        x = tf.keras.layers.Conv2D(filt_lst[0],(3,3),padding="same",activation='relu')(inputs)
        x = tf.keras.layers.Conv2D(filt_lst[0],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.MaxPool2D(2,strides=2)(x)

        x = tf.keras.layers.Conv2D(filt_lst[1],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.Conv2D(filt_lst[1],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.MaxPool2D(2,strides=2)(x)

        x = tf.keras.layers.Conv2D(filt_lst[2],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.Conv2D(filt_lst[2],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.Conv2D(filt_lst[2],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.MaxPool2D(2,strides=2)(x)

        x = tf.keras.layers.Conv2D(filt_lst[3],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.Conv2D(filt_lst[3],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.Conv2D(filt_lst[3],(3,3),padding="same",activation='relu')(x)
        x = tf.keras.layers.MaxPool2D(2,strides=2)(x)
    ...
    def UNET(self, bins, channel, loss_func, opt, metric, reg, filt_num):
        ...
        inputs = tf.keras.layers.Input((bins, bins, channel))
        c1 = tf.keras.layers.Conv2D(filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(inputs)#
        #c1 = tf.keras.layers.Dropout(reg)(c1)
        c1 = tf.keras.layers.Conv2D(filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(c1)#(s)                       
        p1 = tf.keras.layers.MaxPooling2D(2)(c1)
        c2 = tf.keras.layers.Conv2D(2*filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(p1)#(s)
        ...
        c3 = tf.keras.layers.Conv2D(4*filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(p2)#(s)                           
        c3 = tf.keras.layers.Dropout(reg)(c3)
        c3 = tf.keras.layers.Conv2D(4*filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(c3)#(s)                           
        p3 = tf.keras.layers.MaxPooling2D(2)(c3)
        c4 = tf.keras.layers.Conv2D(8*filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(p3)#(s)                           
        ...
        c5 = tf.keras.layers.Conv2D(16*filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(p4)#(s)                           
        c5 = tf.keras.layers.Dropout(reg)(c5)
        c5 = tf.keras.layers.Conv2D(16*filt_num,(3,3),activation='relu', kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(c5)#(s)                           
        p5 = tf.keras.layers.MaxPooling2D(2)(c5)



        u6 = tf.keras.layers.Conv2DTranspose(8*filt_num,2,strides=(2,2),padding='same')(c5)
        u6 = tf.keras.layers.concatenate([u6,c4])
        c6 = tf.keras.layers.Conv2D(8*filt_num,(3,3),activation='relu',kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(u6)
        ...
        u7 = tf.keras.layers.Conv2DTranspose(4*filt_num,2,strides=(2,2),padding='same')(c6)
        u7 = tf.keras.layers.concatenate([u7,c3])
        u6 = tf.keras.layers.Reshape((u6.shape[1],u6.shape[2],1), input_shape=u6.shape)
        c7 = tf.keras.layers.Conv2D(4*filt_num,(3,3),activation='relu',kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(u7)
        ...
        u8 = tf.keras.layers.Conv2DTranspose(2*filt_num,2,strides=(2,2),padding='same')(c7)
        u8 = tf.keras.layers.concatenate([u8,c2])
        c8 = tf.keras.layers.Conv2D(2*filt_num,(3,3),activation='relu',kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(u8)
        ...
        u9 = tf.keras.layers.Conv2DTranspose(filt_num,2,strides=(2,2),padding='same')(c8)
        u9 = tf.keras.layers.concatenate([u9,c1])
        c9 = tf.keras.layers.Conv2D(filt_num,(3,3),activation='relu',kernel_initializer='he_normal', #kernel_regularizer=reg,
        padding='same')(u9)
    ...
    def convolutional_block(self, X, f, filters, stage, block, s = 2):
        ...
        X = tf.keras.layers.Conv2D(F1, (1,1), activation='relu', strides = (s,s), name = conv_name_base + 'conv_0')(X)#, kernel_initializer = glorot_uniform(seed=0))(X)
        #X = BatchNormalization(name = bn_name_base + '2a')(X)
        X = tf.keras.layers.Activation('relu')(X)

        # Second component of main path (≈3 lines)
        X = tf.keras.layers.Conv2D(filters = F2, kernel_size = (f,f), strides = (1,1), padding = 'same', name = conv_name_base + '2b')(X)#, kernel_initializer = glorot_uniform(seed=0))(X)
        X = tf.keras.layers.Activation('relu')(X)
        # Third component of main path (≈2 lines)
        X = tf.keras.layers.Conv2D(filters = F3, kernel_size = (1,1), activation='relu', strides = (1,1), padding = 'valid', name = conv_name_base + 'conv_1')(X)#, kernel_initializer = glorot_uniform(seed=0))(X)
        ##### SHORTCUT PATH #### (≈2 lines)
        X_shortcut = tf.keras.layers.Conv2D(filters = F3, activation='relu', kernel_size = (1,1), strides = (s,s), padding = 'valid', name = conv_name_base + 'conv_shortcut')(X_shortcut)#,
    ...
    def identity_block(self, X, f, filters, stage, block):
        ...
        X_shortcut = X   
        # First component of main path
        X = tf.keras.layers.Conv2D(filters = F1, kernel_size = (1,1), activation='relu', strides = (1,1), padding = 'valid', name = conv_name_base + 'identity_0')(X)#, kernel_initializer = glorot_uniform(seed=0))(X)
        X = tf.keras.layers.Activation('relu')(X)   
        # Second component of main path (≈3 lines)
        X = tf.keras.layers.Conv2D(filters = F2, kernel_size = (f,f), strides = (1,1), padding = 'same', name = conv_name_base + '2b')(X)#, kernel_initializer = glorot_uniform(seed=0))(X)
        X = tf.keras.layers.Activation('relu')(X)
        # Third component of main path (≈2 lines)
        X = tf.keras.layers.Conv2D(filters = F3, kernel_size = (1,1), activation='relu', strides = (1,1), padding = 'valid', name = conv_name_base + 'identity_1')(X)#, kernel_initializer = glorot_uniform(seed=0))(X)
        # Final step: Add shortcut value to main path, and pass it through a RELU activation (≈2 lines)
        X = tf.keras.layers.Add()([X, X_shortcut])
        X = tf.keras.layers.Activation('relu')(X)
        return X
    ...
    def resUnet(self, bins, channel, loss_func, opt, MeTric, reg):
        inputs = tf.keras.layers.Input((bins, bins, channel))
        X = tf.keras.layers.Conv2D(16, (3,3), strides=(1,1), name='conv1',padding='same')(inputs)
    ...
    def res_conv_block(self, x, filter_size, size, dropout, batch_norm=False):
        conv = layers.Conv2D(size, filter_size, padding="same")(x)
        if batch_norm is True:
            conv = layers.BatchNormalization()(conv)
        conv = layers.Activation("relu")(conv)
        ...
        shortcut = layers.Conv2D(size, kernel_size=1, padding="same")(x)
        if batch_norm is True:
    ...
    def gating_signal(self, input, out_size, batch_norm=False):
        x = layers.Conv2D(out_size, 1, padding="same")(input)
        if batch_norm is True:
    ...
    # Getting the x signal to the same shape as the gating signal
        theta_x = layers.Conv2D(inter_shape, 2, strides= 2, padding='same')(x)  # 16
        shape_theta_x = K.int_shape(theta_x)

    # Getting the gating signal to the same number of filters as the inter_shape
        phi_g = layers.Conv2D(inter_shape, 1, padding='same')(gating)
        ...
        result = layers.Conv2D(shape_x[2], 1, padding='same')(y)
        result = layers.Conv2D(1, 1, padding='same')(y)
    ...
    def UNet(self, input_shape, NUM_CLASSES=1, dropout_rate=0.0, batch_norm=True):
        ...
        conv_final = layers.Conv2D(NUM_CLASSES, kernel_size=1)(up_conv_128)
    ...
    def Attention_ResUNet(self, bins, channel, loss_func, opt, metric, FILTER_NUM=64, FILTER_SIZE=3, NUM_CLASSES=1, dropout_rate=0.0, batch_norm=True):
        ...
        inputs = layers.Input((bins, bins, channel))
        #inputs = layers.Input(input_shape, dtype=tf.float32)
        axis = 2
    ...
    def Nest_Net(self, bins, channel, loss_func, opt, metric, nb_filter = [32,64,128,256,512], num_class=1, deep_supervision=False):
        ...
        bn_axis = 2
        img_input = layers.Input(shape=(bins, bins, channel), name='main_input')
    ...
    def unetPP(self,bins,channel,loss_func,opt,metric,reg, nb_filter, filters):
        inputs = Input((bins, bins, channel))
        ...
        c1 = Dropout(reg) (c1)
        p1 = MaxPooling2D(2, strides=(2,2)) (c1)
        ...
        c2 = Dropout(reg) (c2)
        p2 = MaxPooling2D(2, strides=(2,2)) (c2)

        up1_2 = Conv2DTranspose(nb_filter[0], (2,2), strides=(2,2), name='up12', padding='same')(c2)
        conv1_2 = concatenate([up1_2, c1], name='merge12')
        c3 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv1_2)
        c3 = Dropout(reg) (c3)
        c3 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (c3)
        c3 = Dropout(reg) (c3)

        conv3_1 = Conv2D(filters[2], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (p2)
        conv3_1 = Dropout(reg) (conv3_1)
        conv3_1 = Conv2D(filters[2], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv3_1)
        conv3_1 = Dropout(reg) (conv3_1)
        pool3 = MaxPooling2D(2, strides=(2,2), name='pool3')(conv3_1)

        up2_2 = Conv2DTranspose(nb_filter[1], (2,2), strides=(2,2), name='up22', padding='same')(conv3_1)
        conv2_2 = concatenate([up2_2, c2], name='merge22')#, axis=3) #x10
        conv2_2 = Conv2D(filters[1], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv2_2)
        ...
        conv1_3 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv1_3)
        conv1_3 = Dropout(reg) (conv1_3)
        conv1_3 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv1_3)
        conv1_3 = Dropout(reg) (conv1_3)

        conv4_1 = Conv2D(filters[3], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (pool3)
        conv4_1 = Dropout(reg) (conv4_1)
        conv4_1 = Conv2D(filters[3], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv4_1)
        conv4_1 = Dropout(reg) (conv4_1)
        pool4 = MaxPooling2D(2, strides=(2,2), name='pool4')(conv4_1)

        up3_2 = Conv2DTranspose(nb_filter[2], (2,2), strides=(2,2), name='up32', padding='same')(conv4_1)
        conv3_2 = concatenate([up3_2, conv3_1], name='merge32')#, axis=3) #x20
        conv3_2 = Conv2D(filters[2], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv3_2)
        ...
        conv2_3 = Conv2D(filters[1], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv2_3)
        ...
        conv1_4 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv1_4)
        conv1_4 = Dropout(reg) (conv1_4)
        conv1_4 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv1_4)
        conv1_4 = Dropout(reg) (conv1_4)

        conv5_1 = Conv2D(filters[4], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (pool4)
        conv5_1 = Dropout(reg) (conv5_1)
        conv5_1 = Conv2D(filters[4], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv5_1)
        conv5_1 = Dropout(reg) (conv5_1)

        up4_2 = Conv2DTranspose(nb_filter[3], (2,2), strides=(2,2), name='up42', padding='same')(conv5_1)
        conv4_2 = concatenate([up4_2, conv4_1], name='merge42')#, axis=3) #x30
        conv4_2 = Conv2D(filters[3], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv4_2)
        ...
        conv3_3 = Conv2D(filters[2], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv3_3)
        ...
        conv2_4 = Conv2D(filters[1], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv2_4)
        ...
        conv1_5 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv1_5)
        conv1_5 = Dropout(reg) (conv1_5)
        conv1_5 = Conv2D(filters[0], (3,3), activation='relu', kernel_initializer='he_normal', padding='same') (conv1_5)
        conv1_5 = Dropout(reg) (conv1_5)

        nestnet_output_4 = Conv2D(1, 1, activation='relu', kernel_initializer = 'he_normal',  name='output_4', padding='same')(conv1_5)
